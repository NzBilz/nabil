<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex justify-between items-center mb-6">
                        <h3 class="text-lg font-semibold text-gray-800">Formulir Tambah Menu</h3>
                    ...
                    <form action="{{ route('menus.store') }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name') }}" required placeholder="Contoh: Original, Jasmine, Milk Tea"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="number" name="price" id="price" value="{{ old('price') }}" required min="0" placeholder="Contoh: 5000"
                                       class="mt-1 block w-full border-gray-300 focus:border-orange-500 focus:ring-orange-500 rounded-md shadow-sm">
                                @error('price')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="border-t pt-6">
                            ...
                            <div id="recipe-rows-container" class="space-y-3">
                                @if (old('ingredients'))
                                    @foreach (old('ingredients') as $index => $oldIng)
                                        <div class="flex items-center gap-3 bg-slate-50 p-3 rounded border border-gray-200" id="recipe-row-{{ $index }}">
                                            <div class="flex-1">
                                                <select name="ingredients[{{ $index }}][inventory_id]" required
                                                    ...
                                                    @foreach ($inventories as $inv)
                                                        <option value="{{ $inv->id }}" {{ $oldIng['inventory_id'] == $inv->id ? 'selected' : '' }}>
                                            ...
                                            <div class="w-1/3 flex items-center gap-2">
                                                <input type="number" name="ingredients[{{ $index }}][quantity]" required step="0.01" min="0.01" value="{{ $oldIng['quantity'] }}" placeholder="Jumlah"
    ...
    <script>
        ...
        let rowIndex = {{ old('ingredients') ? count(old('ingredients')) : 0 }};
        ...
        // Initialize with one row if empty and no old values
